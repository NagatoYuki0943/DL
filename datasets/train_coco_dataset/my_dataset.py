...
class CocoDetection(data.Dataset):
    ...
    def __init__(self, root, dataset="train", transforms=None):
        ...
        if dataset == "train":
            ...
            coco91to80 = dict(
                [(str(k), idx + 1) for idx, (k, _) in enumerate(coco_classes.items())]
            )
            ...
            # 记录重新编排后的索引以及类别名称关系
            coco80_info = dict(
                [(str(idx + 1), v) for idx, (_, v) in enumerate(coco_classes.items())]
            )
            ...
            assert os.path.exists(coco91to80_path), "file '{}' does not exist.".format(
                coco91to80_path
            )
