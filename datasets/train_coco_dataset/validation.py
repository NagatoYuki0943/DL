...
def main(parser_data):
    ...
    backbone = torch.nn.Sequential(
        *list(vgg_feature._modules.values())[:-1]
    )  # 删除feature中最后的maxpool层
    ...
    for i in range(len(category_index)):
        ...
        voc_map_info_list.append(
            " {:15}: {}".format(category_index[str(i + 1)], stats[1])
        )
...
if __name__ == "__main__":
    ...
    # 检测目标类别数
    parser.add_argument(
        "--num-classes", type=int, default="80", help="number of classes"
    )
    ...
    parser.add_argument(
        "--weights",
        default="./save_weights/model.pth",
        type=str,
