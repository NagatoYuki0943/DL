...
def get_transforms(resize=224):
    ...
    trans = [# transforms.Resize(int(resize/0.875)),         # 224 / 0.875 = 256
             ...
             transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0),   # 参数1为亮度，参数2为对比度，参数3为饱和度，参数4为色相
...
def trans_images(origin: str, new: str, times: int = 4):
    ...
    image_list = [image for image in image_list if image.endswith(("jpg", "jpeg", "png", "bmp"))]
