...
print(len(images))
with open(train_path, mode="a", encoding="utf-8") as f1, open(trainval_path, mode="a", encoding="utf-8") as f2:
    for image in tqdm(images):
        f1.write(image + "\n")
        f2.write(image + "\n")

print(f"空白图片数量为 {len(images)}")
print(f"请手动将 {empty_dir} 目录下的图片和xml文件移动到 VOCdevkit/VOC2007 对应的目录下")
