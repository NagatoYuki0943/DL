...
assert os.path.exists(classes_path), "classes_path does not exist..."
...
def translate_info(file_names: list, save_root: str, classes: dict, train_val='train'):
    ...
    :param save_root:   ./VOC2Yolo 文件保存根目录
    :param classes:  classes list
    :param train_val:   训练还是验证集
    :return:
    """
    ...
    for file in tqdm(file_names, desc="transfer {} file...".format(train_val)):
        # 检查下图像文件是否存在
        ...
        # read xml
        with open(xml_path, mode="r", encoding="utf-8") as fid:
            xml_str = fid.read()
...
def main():
    ...
    translate_info(train_file_names, save_file_root, classes, "train")

    # 读取val.txt中的所有行信息，删除空行
    with open(val_txt_path, "r", encoding="utf-8") as r:
        val_file_names = [i for i in r.read().splitlines() if len(i.strip()) > 0]
    # voc信息转yolo，并将图像文件复制到相应文件夹
    translate_info(val_file_names, save_file_root, classes, "val")
