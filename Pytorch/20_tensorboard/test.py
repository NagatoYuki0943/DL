...
for epoch in range(EPOCH):
    ...
    tb_writer.add_scalar(
        tags[2], optimizer.state_dict()["param_groups"][0]["lr"], epoch
    )
    ...
    if fig is not None:
        tb_writer.add_figure("predictions vs. actuals", figure=fig, global_step=epoch)
    ...
    tb_writer.add_histogram(
        tag="stem/conv", values=model.features[0][0].weight, global_step=epoch
    )
    tb_writer.add_histogram(
        tag="stem/ln", values=model.features[0][1].weight, global_step=epoch
    )
