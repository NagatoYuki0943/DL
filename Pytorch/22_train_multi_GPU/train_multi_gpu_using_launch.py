...
def main(args):
    ...
    # --------------------------------------------------------------#
    #   注意训练集参数是batch_sampler不是batch_size
    #   验证集有一个sampler参数
    # --------------------------------------------------------------#
    train_loader = torch.utils.data.DataLoader(
        train_data_set,
        batch_sampler=train_batch_sampler,  # batch_sampler不是batch_size
        pin_memory=True,
        num_workers=nw,
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        sampler=val_sampler,  # 验证集有一个sampler参数,因为没有用BatchSampler处理
        pin_memory=True,
        num_workers=nw,
    ...
    for epoch in range(args.epochs):
        ...
        if rank == 0:
            ...
            torch.save(
                model.module.state_dict(), "./weights/model-{}.pth".format(epoch)
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data-path", type=str, default="/home/wz/data_set/flower_data/flower_photos"
    )

    # resnet34 官方权重下载地址
    # https://download.pytorch.org/models/resnet34-333f7ec4.pth
    parser.add_argument(
        "--weights", type=str, default="resNet34.pth", help="initial weights path"
    )
    ...
    # --------------------------------------------------------------#
    #   不用设置该参数，开启的进程数(注意不是线程)，会根据nproc_per_node自动设置
    # --------------------------------------------------------------#
    parser.add_argument(
